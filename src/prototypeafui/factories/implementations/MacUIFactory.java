...
public class MacUIFactory implements UIFactory {
	private static final String PRE_FIX = "Mac";
	...
	public Window getWindow(Color color) {
		if (!this.windows.containsKey(color))
			this.windows.put(color, new ConcreteWindow(PRE_FIX + " Window", 200, 200, color, Border.ROUND));
	...
	public Button getButton(Color color) {
		if (!this.buttons.containsKey(color))
			this.buttons.put(color, new ConcreteButton(PRE_FIX + " Button", 50, 30, color, Border.ROUND, Font.COMIC));
	...
	public TextBox getTextBox(Font font) {
		if (!this.textBoxes.containsKey(font))
			this.textBoxes.put(font, new ConcreteTextBox(PRE_FIX + " TextBox", 170, 100, Color.BLACK, font, TextFormat.JUSTIFIED));
